...
pub async fn connect_on_connection(
    request_id: Uuid,
    ...
    debug!(
        "QUIC connect: Starting tunnel connection for request {} (existing connection stable_id={})",
        request_id,
    ...
    // Open bi-directional stream on the provided connection
    debug!("QUIC connect: Opening bidirectional stream");
    let (mut send, mut recv): (SendStream, RecvStream) = connection
        .open_bi()
        .await
        .context("failed to open QUIC stream on existing connection")?;
    ...
    match timeout(handshake_timeout, send.write_all(&buf)).await {
        ...
        Ok(Err(e)) => return Err(e).context("failed to send HTTP request"),
        Err(_) => return Err(anyhow!("QUIC handshake write timed out")),
    }
    ...
    match timeout(handshake_timeout, handshake_result).await {
        ...
        Ok(Err(e)) => Err(e),
        Err(_) => Err(anyhow!("QUIC handshake timed out")),
    }
}
